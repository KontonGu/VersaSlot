...
// 获取任务名
const std::string &Task::getTaskName() const { return name; }
...
// 获取应用名
const std::string &Application::getAppName() const { return name; }
...
// //   // 生成的应用程序
// //   AppGenStruct app1 = {appgenerator_LENet, 20, 2, 9.0, 9};
// //   AppGenStruct app2 = {appgenerator_OF, 20, 6, 3.0, 3};
// //   AppGenStruct app3 = {appgenerator_3DR, 20, 2, 9.0, 9};
// //   AppGenStruct app4 = {appgenerator_IC, 20, 5, 9.0, 9};
// //   AppGenStruct app5 = {appgenerator_LENet, 20, 2, 9.0, 9};
// //   AppGenStruct app6 = {appgenerator_3DR, 20, 2, 9.0, 9};
// //   AppGenStruct app7 = {appgenerator_OF, 20, 6, 3.0, 3};
// //   AppGenStruct app8 = {appgenerator_IC, 20, 5, 9.0, 9};
// //   AppGenStruct app9 = {appgenerator_3DR, 20, 2, 9.0, 9};
// //   AppGenStruct app10 = {appgenerator_LENet, 20, 2, 9.0, 9};
// //   AppGenStruct app11 = {appgenerator_3DR, 20, 2, 9.0, 9};
// //   AppGenStruct app12 = {appgenerator_IC, 20, 5, 9.0, 9};
// //   AppGenStruct app13 = {appgenerator_3DR, 20, 2, 9.0, 9};
// //   AppGenStruct app14 = {appgenerator_LENet, 20, 2, 9.0, 9};
// //   AppGenStruct app15 = {appgenerator_3DR, 20, 2, 9.0, 9};
// //   AppGenStruct app16 = {appgenerator_IC, 20, 5, 9.0, 9};
// //   AppGenStruct app17 = {appgenerator_LENet, 20, 2, 9.0, 9};
// //   AppGenStruct app18 = {appgenerator_3DR, 20, 2, 9.0, 9};
// //   AppGenStruct app19 = {appgenerator_IC, 20, 5, 9.0, 9};
// //   AppGenStruct app20 = {appgenerator_LENet, 20, 2, 9.0, 9};
...
void initialize_all_apps() {
  ...
  AppGenStruct app21 = {appgenerator_OF, 10, 3, 3.0, 3};
  ...
  for (auto it = pending_apps.begin(); it != pending_apps.end();) {
    ...
    if (cur_app->priority >= threshold) {
      ...
      snprintf(log_message, sizeof(log_message),
               "Moving App %s from pending to candidate with priority %f, "
               "threshold was %f",
...
void execute_tasks(std::list<Task *> &taskQueue, std::vector<Slot *> &slots,
  ...
  for (auto &slot : slots) {
    ...
    if (task != nullptr && slot->isSlotIdle()) {
      // 确认任务尚未开始任何批次时设置 IP 参数
      if (task->current_batch_index == 0 && !task->batch_infos[0].isCompleted) {
        // 获取参数并设置到 slot
      }
      ...
      if (task->current_batch_index > 0) {
        ...
        if (batch_index >= 0 && !task->batch_infos[batch_index].isCompleted) {
          ...
          // 记录批次完成日志
          const std::string &appName = task->getAppName()->getAppName();
          const std::string &taskName = task->getTaskName();
          char batch_end_message[128];
          sprintf(batch_end_message,
                  "App %s, Task %s, Batch %d finished at Slot %d",
                  appName.c_str(), taskName.c_str(), batch_index + 1,
                  slot->getSlotNr());
          print_log("INFO", batch_end_message);
      ...
      if (task->current_batch_index < task->batch) {
        ...
        if (task->canStartBatch(task->current_batch_index)) {
          ...
          sprintf(batch_start_message,
                  ...
                  appName.c_str(), taskName.c_str(),
                  task->current_batch_index + 1, slot->getSlotNr());
        ...
        sprintf(end_time_message,
                ...
                appName.c_str(), taskName.c_str(), slot->getSlotNr());
  ...
  for (Task *task : taskQueue) {
    ...
    if (pr_task->pr_status == 3) {
      ...
      snprintf(time_log_message, sizeof(time_log_message),
               "Slot %d finished PR", pr_task->slot_nr);
    ...
    // 查找一个空闲的 Slot，跳过正在 PR 的 Slot
    Slot *available_slot = nullptr;
    for (auto &slot : slots) {
      // 判断是否当前 PR 任务涉及到当前 slot
      if (pr_task->slot_nr == slot->getSlotNr()) {
        // 如果当前 PR 任务涉及此 slot，则要求 pr_status 为 0（空闲状态）
        if (pr_task->pr_status == 0 && slot->isSlotIdle() &&
            slot->curTask == nullptr) {
          available_slot = slot;
          break;
        }
      } else {
        // 如果当前 PR 任务不涉及此 slot，则只检查 slot 的空闲状态
    ...
    // 将 task->getBitName() 返回的字符串内容复制到 bitstream_file_name 数组
