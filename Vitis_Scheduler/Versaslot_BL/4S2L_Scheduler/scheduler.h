...
typedef struct {
  char bitstream_file_name[128]; // 128字节
  int slot_nr;                   // 4字节
  int slot_type;                 // 4字节
  uint32_t decouple_value;       // 4字节
  volatile uint8_t pr_status;    // 1字节
  volatile uint8_t init_success; // 1字节
  uint8_t padding[2];            // 2字节填充，确保总大小为144字节
...
public:
  ...
  std::vector<Task *> dependencies3in1P; // 依赖的任务列表
  std::vector<Task *> dependencies3in1S; // 依赖的任务列表
...
public:
  ...
  int unique_ID;
  int slot_dep_fail; // slot预加载时task依赖不满足的计数
  int isAllocated;   // 0 = 未分配 1 = 分配到大核 2 = 分配到小核
  bool isAppStart;
